...
public abstract class Span implements SpanCustomizer {
  ...
  /**
   * @deprecated Use {@link #remoteServiceName(String)} {@link #parseRemoteIpAndPort(String, int)}.
  ...
  @Deprecated public Span remoteEndpoint(Endpoint endpoint) {
    ...
    parseRemoteIpAndPort(ip, endpoint.portAsInt());
    return this;
  }
   ...
   *
   * @see #parseRemoteIpAndPort(String, int)
   */
   ...
   * <pre>{@code
   * if (span.parseRemoteIpAndPort(address.getHostAddress(), target.getPort())) return;
   * span.parseRemoteIpAndPort(address.getHostName(), target.getPort());
   * }</pre>
   *
  ...
  // weight for little benefit. If this changes, we can overload with
  public abstract boolean parseRemoteIpAndPort(@Nullable String remoteIp, int remotePort);

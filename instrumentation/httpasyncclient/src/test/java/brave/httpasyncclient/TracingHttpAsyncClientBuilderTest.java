...
public class TracingHttpAsyncClientBuilderTest {
  ...
  @Test public void parseTargetAddress_prefersAddress() throws UnknownHostException {
    ...
    when(span.parseRemoteIpAndPort("1.2.3.4", -1)).thenReturn(true);
    ...
    verify(span).isNoop();
    verify(span).parseRemoteIpAndPort("1.2.3.4", -1);
    verifyNoMoreInteractions(span);
  }
  ...
  @Test public void parseTargetAddress_ipAndPortFromHost() {
    when(span.isNoop()).thenReturn(false);
    when(span.parseRemoteIpAndPort("1.2.3.4", 9999)).thenReturn(true);
    ...
    verify(span).isNoop();
    verify(span).parseRemoteIpAndPort("1.2.3.4", 9999);
    verifyNoMoreInteractions(span);
  }
}

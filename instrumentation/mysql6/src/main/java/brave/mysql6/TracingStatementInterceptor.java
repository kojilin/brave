...
public class TracingStatementInterceptor implements StatementInterceptor {
  ...
  public <T extends Resultset> T preProcess(String sql, Statement interceptedStatement) {
    ...
    parseServerIpPort(connection, span);
    span.start();
    return null;
  }
   ...
   */
  static void parseServerIpPort(MysqlConnection connection, Span span) {
    try {
      ...
      if (host != null) {
        span.parseRemoteIpAndPort(host, url.getPort() == -1 ? 3306 : url.getPort());
      }
    } catch (Exception e) {

...
final class TracingJdbcEventListener extends SimpleJdbcEventListener {
  ...
  @Override public void onBeforeAnyExecute(StatementInformation info) {
    ...
    parseServerIpPort(info.getConnectionInformation().getConnection(), span);
    span.start();
  }
   ...
   */
  void parseServerIpPort(Connection connection, Span span) {
    try {
      ...
      span.parseRemoteIpAndPort(url.getHost(), url.getPort());
    } catch (Exception e) {

...
public class HttpServletAdapter extends HttpServerAdapter<HttpServletRequest, HttpServletResponse> {
   ...
   * address} if the {@link HttpServerAdapter#parseClientIpPort default parsing} fails.
   */
  @Override public boolean parseClientIpPort(HttpServletRequest req, Span span) {
    if (super.parseClientIpPort(req, span)) return true;
    return span.parseRemoteIpAndPort(req.getRemoteAddr(), req.getRemotePort());
  }
